 ...
 public:
  void update(float speed, Angle steeringAngle, float dt) {
    if (steeringAngle.radians() != 0.0f) {
      float R =
          wheelbase / std::tan(steeringAngle.radians()) + trackWidth / 2.0f;
      angularVelocity = AngularVelocity::fromRadiansPerSecond(speed / R);
      heading += angularVelocity * dt;
    } else {
      angularVelocity = AngularVelocity{};
    }
    pos = pos + Pos2(speed * std::cos(heading.radians()),
                     speed * std::sin(heading.radians())) *
                    dt;
  }
...
class VehicleBuilder {
 public:
  ...
  Vehicle build() {
    ...
    if (!hasHeading)
      throw std::invalid_argument("VehicleBuilder: heading is required");
    if (!hasWheelbase)
      throw std::invalid_argument("VehicleBuilder: wheelbase is required");
    if (!hasTrackWidth)
      throw std::invalid_argument("VehicleBuilder: trackWidth is required");
